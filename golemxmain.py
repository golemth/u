...
bot = telebot.TeleBot('8175288335:AAFWmY-aeeLeX8ETmATLBg6cHxEbKTQCR4w')
...
def start_command(message):
    ...
    bot.reply_to(message, "𝙒𝙀𝙇𝘾𝙊𝙈𝙀 𝙏𝙊 𝙋𝘼𝙄𝘿 𝘿𝘿𝙊𝙎 𝘽𝙊𝙏!", reply_markup=markup)
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        response = "⛔️ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM @Gx7_Owner"
        bot.reply_to(message, response)
...
def status_command(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "⛔️ 𝗬𝗼𝘂 𝗮𝗿𝗲 𝗻𝗼𝘁 𝗮𝗻 𝗮𝗱𝗺𝗶𝗻 @Gx7_Owner.")
...
def upload_to_terminal(message):
    """Handle file upload to terminal for admins @Gx7_Owner."""
    user_id = str(message.chat.id)
...
def add_user_command(message):
    if str(message.chat.id) not in admin_id:
        bot.reply_to(message, "⛔ 𝗔𝗰𝗰𝗲𝘀𝘀 𝗗𝗲𝗻𝗶𝗲𝗱: 𝗔𝗱𝗺𝗶𝗻 𝗼𝗻𝗹𝘆 𝗰𝗼𝗺𝗺𝗮𝗻𝗱 DM @Gx7_Owner")
        return
...
if __name__ == "__main__":
    print("✅ BOT IS STARTING FILE DEVELOPER @Gx7_Owner!... ")
    while True:
        load_data()
        try:
